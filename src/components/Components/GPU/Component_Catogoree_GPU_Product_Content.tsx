...
function Component_Catogoree_GPU_Product_Content({score}:any) {
    const [data, setProps] = useState<Data[]>([]);

    useEffect(() => {
        const fetchData = async () => {
            try {
                const response = await axios.get(`http://localhost:5050/products/filter_by_type?req_type=GPU&size=16&page=${score}`);
