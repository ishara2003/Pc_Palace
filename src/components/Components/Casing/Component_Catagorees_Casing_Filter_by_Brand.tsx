...
function Component_Catagorees_Casing_Filter_by_Brand() {
    ...
    const [casingcheckboxStates, setCasingcheckboxStates]
        = useState<casingCheckboxData[]>(initialcasingCheckboxes);
            ...
            {casingcheckboxStates.map((checkbox, index) => (
                <div key={index}>
                    ...
                    <label className={'text-center cursor-pointer'}
                           htmlFor={`casingcheckbox${index + 1}`}>{checkbox.casingName}</label>
                </div>
            ))}
        </div>
    );
