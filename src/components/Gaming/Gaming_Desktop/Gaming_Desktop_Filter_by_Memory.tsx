...
function Gaming_Desktop_Filter_by_Memory() {
        ...
        <div className={'filter_css'}>
            ...
            {ramcheckboxStates.map((checkbox, index) => (
                <div key={index}>
                    ...
                    />
                    <label className={'text-center cursor-pointer'} htmlFor={`ramcheckbox${index + 1}`}>{checkbox.ramName}</label>
                </div>
            ))}
        </div>
    )
...
export default Gaming_Desktop_Filter_by_Memory;